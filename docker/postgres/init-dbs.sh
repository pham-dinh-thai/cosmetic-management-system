...
psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" <<-SQL
  ...
  CREATE USER ${INVOICE_DB_USER} WITH PASSWORD '${INVOICE_DB_PASSWORD}';
  CREATE USER ${BASKET_DB_USER} WITH PASSWORD '${BASKET_DB_PASSWORD}';
  CREATE USER ${RECEIPT_DB_USER} WITH PASSWORD '${RECEIPT_DB_PASSWORD}';
...
psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" <<-SQL
  ...
  SELECT 'CREATE DATABASE "$ORDER_DB_NAME" OWNER "$ORDER_DB_USER"'
  WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$ORDER_DB_NAME')\gexec
  SELECT 'CREATE DATABASE "$INVOICE_DB_NAME" OWNER "$INVOICE_DB_USER"'
  WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$INVOICE_DB_NAME')\gexec
  SELECT 'CREATE DATABASE "$BASKET_DB_NAME" OWNER "$BASKET_DB_USER"'
  WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$BASKET_DB_NAME')\gexec
  SELECT 'CREATE DATABASE "$RECEIPT_DB_NAME" OWNER "$RECEIPT_DB_USER"'
  WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$RECEIPT_DB_NAME')\gexec
  SELECT 'CREATE DATABASE "$AUDIT_DB_NAME" OWNER "$AUDIT_DB_USER"'
...
for pair in \
  ...
  "$PURCHASE_DB_NAME:$PURCHASE_DB_USER" \
  "$ORDER_DB_NAME:$ORDER_DB_USER" \
  "$INVOICE_DB_NAME:$INVOICE_DB_USER" \
  "$BASKET_DB_NAME:$BASKET_DB_USER" \
  "$RECEIPT_DB_NAME:$RECEIPT_DB_USER" \
  "$AUDIT_DB_NAME:$AUDIT_DB_USER"; do
  db="${pair%%:*}"
  owner="${pair##*:}"
...
grant_schema_privileges "$ORDER_DB_NAME" "$ORDER_DB_USER"
grant_schema_privileges "$INVOICE_DB_NAME" "$INVOICE_DB_USER"
grant_schema_privileges "$BASKET_DB_NAME" "$BASKET_DB_USER"
grant_schema_privileges "$RECEIPT_DB_NAME" "$RECEIPT_DB_USER"
grant_schema_privileges "$AUDIT_DB_NAME" "$AUDIT_DB_USER"
