...
export class Migration20260825174219 extends Migration {
  ...
  override up(): void | Promise<void> {
    ...
    this.addSql(
      `alter table "employees" alter column "status" type text using ("status"::text);`,
    );
    this.addSql(
      `alter table "employees" add constraint "employees_status_check" check ("status" in ('ACTIVE', 'INACTIVE', 'ON_LEAVE', 'TERMINATED'));`,
    );
    this.addSql(
  ...
  override down(): void | Promise<void> {
    ...
    this.addSql(
      `alter table "employees" drop constraint "employees_position_check";`,
    );
    this.addSql(`alter table "employees" drop column "position";`);
    this.addSql(
      `alter table "employees" alter column "status" type varchar(255) using ("status"::varchar(255));`,
    );
