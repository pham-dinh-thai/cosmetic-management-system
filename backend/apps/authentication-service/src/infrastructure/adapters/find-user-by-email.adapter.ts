import { Injectable } from '@nestjs/common';
import {
  FindUserByEmailReadModel,
  IFindUserByEmailPort,
} from '../../application/ports/find-user-by-email.port';
import { ConfigService } from '@nestjs/config';

@Injectable()
export class FindUserByEmailAdapter implements IFindUserByEmailPort {
  private readonly baseUrl: string;

  public constructor(private readonly config: ConfigService) {
    const url = this.config.get<string>('USER_SERVICE_URL');

    if (!url) {
      throw new Error('USER_SERVICE_URL is not configured');
    }

    this.baseUrl = url;
  }

  public async execute(
    email: string,
  ): Promise<FindUserByEmailReadModel | null> {
    const response = await fetch(
      `${this.baseUrl}/api/internal/users/by-email/${email}`,
    );

    if (response.status === 404) {
      return null;
    }

    if (!response.ok) {
      const body = await response.text().catch(() => '');
      throw new Error(
        `User service returned ${response.status}: ${response.statusText}${body ? ` - ${body}` : ''}`,
      );
    }

    const text = await response.text();

    if (!text) {
      return null;
    }

    const data = JSON.parse(text) as {
      id: string;
      roleId: string;
      isActive: boolean;
    };

    return new FindUserByEmailReadModel(data.id, data.roleId, data.isActive);
  }
}
