...
export class Migration20260904180507 extends Migration {
  ...
  override up(): void | Promise<void> {
    this.addSql(`create table "purchase_transactions" ("id" uuid not null default gen_random_uuid(), "purchase_order_id" uuid not null, "variant_id" varchar(255) not null, "quantity" int not null default 1, "unit_price" numeric(12,2) not null, "subtotal" numeric(12,2) not null default 0, "employee_id" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, primary key ("id"));`);

    this.addSql(`alter table "purchase_transactions" add constraint "purchase_transactions_purchase_order_id_foreign" foreign key ("purchase_order_id") references "purchase_orders" ("id") on delete cascade;`);
