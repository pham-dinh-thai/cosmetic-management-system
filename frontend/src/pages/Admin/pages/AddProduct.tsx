...
const AddProduct: React.FC<AddProductProps> = ({ onBack }) => {
  ...
  const handleVariantChange = (index: number, field: keyof CreateVariantPayload, value: string | number) => {
  ...
  const addVariant = () => {
    setVariants([...variants, { name: "", price: 0, color: "", volume: "", costPrice: 0 }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const payload: CreateCosmeticPayload = {
    ...
    <div className="flex flex-col gap-8 max-w-4xl pb-16">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
        ...
        <Card className="p-8 flex flex-col gap-6">
          <h2 className="text-[20px] text-[--color-forest-depths]" style={{ fontWeight: 350 }}>Thông tin cơ bản</h2>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h2 className="text-[20px] text-[--color-forest-depths]" style={{ fontWeight: 350 }}>Biến thể (Variants)</h2>
          ...
          {variants.map((variant, index) => (
            <Card key={index} className="p-6 flex flex-col gap-6 bg-transparent border-[1.5px] border-[#b3b3b3]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="flex flex-col gap-2 md:col-span-6">
                  <label className="text-[12px] font-medium text-[#666666] uppercase tracking-[0.1em]">Giá gốc (tuỳ chọn)</label>
                  <Input 
                    type="number"
                    min={0}
        ...
        <div className="pt-8 border-t border-[--color-warm-stone] flex justify-end">
          <Button type="submit" variant="primary" className="min-w-[200px]" disabled={loading}>
